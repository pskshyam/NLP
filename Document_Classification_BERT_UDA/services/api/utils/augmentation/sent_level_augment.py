...
from services.api.utils.augmentation import word_level_augment
...
def run_augment(
    ...
    start, end, dst_tot_size, bt_filepath):
  """Sentence level augmentations. Used before augmentation."""
  if aug_ops:
    if aug_ops.startswith("bt"):
      examples = back_translation(
          examples, aug_ops, sub_set, aug_copy_num, start, end, dst_tot_size, bt_filepath)
    else:
      pass
  return examples
